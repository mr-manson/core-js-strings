...
function isPalindrome(str) {
  const noSpacesStrToLowerCase = str.toLowerCase().replace(/[^a-zA-Z0-9]/g, '');
  return (
    noSpacesStrToLowerCase.split('').reverse().join('') ===
    noSpacesStrToLowerCase
  );
}
...
function encodeToRot13(str) {
  ...
  return str
    ...
    .map((letter) => {
      ...
      return position > -1 ? output[position] : letter;
    })
    .join('');
}
